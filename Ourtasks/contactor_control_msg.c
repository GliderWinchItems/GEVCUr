...
static struct LCDTASK_LINEBUF*   pbuflcdi2cm1; // Ptr to LCDI2C unit 4x20 msg buffer #1
//static struct LCDMSGSET lcdi2cfunc1;
static void lcdi2cmsgm1(union LCDSETVAR u){lcdi2cprintf(&pbuflcdi2cm1,CNCTRLCDA,0," : %2i %2i ",u.u8[0],u.u8[2]);}//(p->cd.uc[0] & 0xf),(p->cd.uc[2] & 0xf));
...
void contactor_control_msg(struct CANRCVBUF* p)
{
	...
	if (pbuflcdi2cm1 == NULL)
  	...
  	if (pbuflcdi2cm1 == NULL) morse_trap(822); 

	if (p->dlc != 3)
	{
//                                                   12345678901234567890             
		lcdprintf(&gevcufunction.pbuflcd2,CNCTRLCDB,0,"CONTCTR ER dlc != 3");
		lcdi2cputs         (&pbuflcdi2cm1,CNCTRLCDB,0,"CONTCTR ER dlc != 3");
		return;
	}
